...
class Page(qtw.QWidget):
	...
	def __init__(self, window: qtw.QWidget):
		...
		self.layout.addWidget(TextInputOption(widget, self, "spo_cli_sec", "Spotify API secret", "API secret for Spotify developer id. \n developer.spotify.com/", False, ""))
		self.layout.addWidget(TextInputOption(widget, self, "spo_cli_red", "Spotify API redirect URL", "API redirect url. \n developer.spotify.com/ \n Example: 'http://127.0.0.1:8888/callback' " , False, ""))
		self.layout.addWidget(SpinBoxOption(widget, self, "SMLD-mutithreading", "Number of concurrent downloads", 2, 16))
		self.layout.addWidget(DropdownOption(widget, self, "SMLD-source", "Select the media download source", ["YouTube Music", "YouTube"]))
		self.layout.addWidget(BooleanOption(widget, self, "SMLD-useytmetadata", "Get metadata from Music Brainz."))
