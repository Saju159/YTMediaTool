...
def get_playlist_details(playlist_url):
    ...
    if not playlist_items:
        ...
        for track in range(len(playlist_items)):
            ...
            video_id = playlist_items[track]['videoId']
            song_title = playlist_items[track]['title']
            artist = playlist_items[track]['artists']
            album = playlist_items[track]['album']

            artist = str(artist).split(":")[1].split(",")[0].replace("'", "")
            if not album == None:
                album = str(album).split(":")[1].split(",")[0].replace("'", "")
            else:
                album = "[Unkown Album]"
...
if __name__ == "__main__":
    ...
    get_playlist_details(playlist_url)
